        ...
        if status == 'like_and_comment':
            ...
            for i in comments:
                ...
                try:
                    response = requests.post(insert_comment)
                    if response.status_code == 200:
                        insert_comment = f"{url}/api/update/{project}/like-and-comment-reply-comment?reaction_type={reaction}&link={link}&comment_text={quote_plus(i)}"
                        try:
                            response = requests.post(insert_comment)
                            if response.status_code == 200:
                                print(f"✅ INSERT สำเร็จ: {i}")
            ...
            for i in comments:
                ...
                try:
                    response = requests.post(insert_comment)
                    if response.status_code == 200:
                        insert_project = f"{url}/api/update/{project}/like-reel-and-comment-reel?reaction_type={reaction}&link={link}&comment_text={quote_plus(i)}"
                        try:
            ...
            api_insert = f"{url}/api/update/{project}/subscribe-id?subscribee_id={subscribee_id}"
            ...
            try:
                response = requests.post(api_insert)
                if response.status_code == 200:
                    url_insert = f"{url}/api/update/{project}/shared-link-text?status_text={status}&status_link={status_link}"
                    try:
